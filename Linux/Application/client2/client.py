...
class Client:
    ...
    def execFile():
        ...
        with open(Client.OUTPUT_FILE, 'w') as f:
            process = subprocess.run(['python3', Client.PY_FILE], stdout=f, universal_newlines=True)
    ...
    def becomeIdler(self, server_ip, server_port):
        ...
        self.server_proc = subprocess.Popen(["gnome-terminal", "-x", "python3", "{}".format(Client.CLIENT_AS_SERVER_FILEPATH), "{}".format(self.client_as_server_ip), "{}".format(self.client_as_server_port)])
